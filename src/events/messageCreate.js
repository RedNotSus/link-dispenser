...
const { AFK } = require("../database/index");
  ...
  async execute(message, client) {
    ...
    try {
      ...
      if (afkData) {
        ...
        const welcomeBackMessage = new EmbedBuilder()
          ...
          .setDescription(
            `\`✅\` Welcome back, ${message.author.username}! I have removed your AFK status.`
          )
          .setTimestamp();
      ...
      for (const user of message.mentions.users.values()) {
        ...
        if (mentionedAFK) {
          const afkMentionMessage = new EmbedBuilder()
            ...
            .setDescription(
              `\`💤\` ${user.username} is currently AFK. Reason: ${
                mentionedAFK.reason || "No reason provided."
              }`
            )
            .setTimestamp();
    ...
    if (!command) {
      try {
        ...
        client.logs.error(
          `[PREFIX_ERROR] Error sending 'cannot find prefix' embed.`,
          error
        );
      }
    }
    ...
    try {
      ...
      console.error(
        `${
          color.red
        }[${getTimestamp()}] [MESSAGE_CREATE] Error while executing command. \n${
          color.red
