...
client.on("ready", async (client) => {
  try {
    setInterval(() => {
      let activities = [
        { type: "Watching", name: `Movies and TV Shows on 55GMS` },
        {
          type: "Watching",
        ...
        { type: "Playing", name: `250+ Games on 55GMS` },
      ];
...
client.on("ready", () => {
  try {
    ...
    client.logs.success(
      `[STATUS] Bot status loaded as ${client.config.status}.`
    );
  } catch (error) {
...
client.on("guildDelete", async (guild) => {
  try {
    ...
    console.log(
      `${color.blue}[${getTimestamp()}]${color.reset} [GUILD_DELETE] ${
        client.user.username
      } has left a guild. \n${color.blue}> GuildName: ${
        guild.name
      } \n> GuildID: ${guild.id} \n> Owner: ${
        theowner
          ? `${theowner.tag} (${theowner.id})`
  ...
  if (!interaction.isChatInputCommand()) return;
    ...
    try {
      const channel = await client.channels.cache.get(
        client.config.slashCommandLoggingChannel
      );
      ...
      console.log(
        `${color.torquise}[${getTimestamp()}]${
          color.reset
        } [SLASH_COMMAND_USED] ${user} has used a command. \n${
          color.torquise
        }> Server: ${server} \n> Command: ${interaction} \n> User: ${user} \n> UserID: ${userID}`
      );
    } catch (error) {
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (!message.author.bot && message.content.startsWith(prefix)) {
    try {
      const channel = await client.channels.cache.get(
        client.config.prefixCommandLoggingChannel
      );
      ...
      await channel.send({ embeds: [embed] });
      console.log(
        `${color.purple}[${getTimestamp()}]${
          color.reset
        } [PREFIX_COMMAND_USED] ${user} has used a command. \n${
          color.purple
        }> Server: ${server} \n> Command: ${
          message.content
        } \n> User: ${user} \n> UserID: ${userID}`
