...
const randomTopic = `https://api.some-random-api.com/facts{${Math.floor(
  Math.random() * factTypes.length
)}}`;
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      const url = topic ? `${BASE_FACTS_URL}/${topic}` : randomTopic;
      ...
      if (!factText) {
        return interaction.editReply({
          content:
            "`❌` Could not get a fact right now, please try again later.",
          ephemeral: true,
        });
      }
