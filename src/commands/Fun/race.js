...
const languages = {
  vi: {
    race_started:
      "🏁 Đã bắt đầu cuộc đua! Ấn `join` để tham gia! 🏁\n**➜** Cuộc đua sẽ bắt đầu sau **1 phút** nữa!!!!",
  ...
  en: {
    race_started:
      "🏁 The race has started! Type `join` to participate! 🏁\n**➜** The race will start in **1 minute**!!!!",
...
function applyRandomEvent(raceLine, weather, language, interaction) {
  ...
  if (Math.random() < 0.2) {
    // 20% chance for a random event
    raceLine *= randomEvent.effect;
    interaction.channel.send(
      languages[language].events[randomEvent.message].replace(
        "{player}",
        raceLine.split(" ")[2]
      )
    );
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("language")
        .setDescription("Select language (default: en)")
        .setRequired(false)
        .addChoices(
  ...
  async execute(interaction, client) {
    ...
    collector.on("collect", (m) => {
      if (!participants.includes(m.author.id)) {
        ...
        userEmos[`<@${m.author.id}>`] =
          customEmoji ||
          gamemodes[gamemode][
            Math.floor(Math.random() * gamemodes[gamemode].length)
          ];
        const joinEmbed = new EmbedBuilder()
          .setColor("#00FF00")
          .setDescription(
            languages[language].player_joined.replace("{player}", m.author.id)
          );
    ...
    collector.on("end", async () => {
      ...
      const playersEmbed = new EmbedBuilder()
        .setColor("#00FF00")
        .setDescription(
          languages[language].participants.replace(
            "{participants}",
            playerMentions
          )
        );
      ...
      const interval = setInterval(async () => {
        ...
        const raceProgressEmbed = new EmbedBuilder()
          .setColor("#00FF00")
          .setTitle(
            language === "vi" ? "📍 Tiến trình cuộc đua" : "📍 Race Progress"
          )
          .setDescription(raceMsg)
          .addFields({
            name: language === "vi" ? "Thời tiết" : "Weather",
            value: languages[language].weather_options[weather],
          })
          .setFooter({
            text:
              language === "vi"
      ...
      function move(raceMsg, interval) {
        ...
        if (!raceLines.every((line) => line.includes("🚩"))) {
          const updatedLines = raceLines.map((line) => {
            if (!line.includes("🚩")) {
              ...
              if (
                ...
                finishOrder.length < participants.length / 2
              ) {
          ...
          updatedLines.sort((a, b) => {
            const aDistance =
              a.indexOf("🚩") - a.indexOf(userEmos[a.split(" ")[distance + 2]]);
            const bDistance =
              b.indexOf("🚩") - b.indexOf(userEmos[b.split(" ")[distance + 2]]);
          ...
          const winnerEmbed = new EmbedBuilder()
            .setColor("#ffda35")
            .setDescription(
              languages[language].race_result.replace("{winner}", winner)
            );
          interaction.channel.send({ embeds: [winnerEmbed] });

          const leaderboard = finishOrder
            .slice(0, 10)
            .map((playerTag, index) => `${index + 1}. ${playerTag}`)
            .join("\n");
