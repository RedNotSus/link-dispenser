...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    function getTotalTime(time) {
      ...
      switch (unit) {
        ...
        case "d":
          ...
          break;
        default:
          return { error: "Invalid time unit. Use s, m, h, or d." };
      }
    ...
    try {
      ...
      if (error) {
        return interaction.reply({
          content: `\`❌\` Invalid time format: ${error}`,
          flags: 64,
        });
      }
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `\`✅\` Successfully muted **${member.user.tag}** for **${
            timeString ?? "1d"
          }**.`
        );
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `\`❌\` An error occurred while muting the user: ${err.message}`
        );
