...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (
      !requiredBotPermissions.every((permission) =>
        botPermissions.has(permission)
      )
    ) {
      return interaction.reply({
        content:
          "`❌` I do not have the necessary permissions to lock channels.",
        flags: 64,
      });
    }
    ...
    if (!interaction.member.permissions.has(Flags.ManageChannels)) {
      return interaction.reply({
        content:
          "`❌` You do not have the necessary permissions to lock channels.",
        flags: 64,
      });
    }
    ...
    try {
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `\`❌\` An error occurred while locking the channel: ${err.message}`
        );
