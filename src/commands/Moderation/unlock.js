...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (
      ...
      return interaction.reply({
        content:
          "`❌` I do not have the necessary permissions (Manage Channels, Send Messages, Embed Links) to unlock the channel.",
        flags: 64,
      });
    }
    ...
    try {
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `\`✅\` The channel **${channel.name}** has been unlocked.`
        )
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `\`❌\` An error occurred while unlocking the channel: ${err.message}`
        );
