...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName("send")
    .setDescription(
      "📨 Sends a message via the bot, either to a channel or a user"
    )
    ...
    .addChannelOption((option) =>
      option
        .setName("channel")
        .setDescription(
          "The channel to send the message to (if type is Channel)"
        )
    ...
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription(
          "The user to send the message to (if type is Direct Message)"
        )
        .setRequired(false)
    ),
  ...
  async execute(interaction, client) {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              "`❌` You must specify a channel when type is set to Channel."
            ),
        ],
        flags: 64,
      });
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor(client.config.embedERROR)
            .setDescription(
              "`❌` You must specify a user when type is set to Direct Message."
            ),
        ],
        flags: 64,
      });
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor(client.config.embedERROR)
            .setDescription(
              "`❌` You cannot specify a user when type is set to Channel."
            ),
        ],
        flags: 64,
      });
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor(client.config.embedERROR)
            .setDescription(
              "`❌` You cannot specify a channel when type is set to Direct Message."
            ),
        ],
        flags: 64,
      });
    ...
    try {
      ...
      const embedMode = response.fields
        .getTextInputValue("embed_mode")
        ?.toLowerCase();
      const embedTitle = response.fields.getTextInputValue("embed_title");
      let embedColor =
        response.fields.getTextInputValue("embed_color") ||
        client.config.embedSuccess ||
        "#4052d6";
        ...
        return response.reply({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embedERROR)
              .setDescription(
                '`❌` Invalid embed mode. Please enter "on" or "off".'
              ),
          ],
          flags: 64,
        });
        ...
        await response.reply({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embedERROR)
              .setDescription(
                "`❌` Invalid color format. Using default color."
              ),
          ],
          flags: 64,
        });
