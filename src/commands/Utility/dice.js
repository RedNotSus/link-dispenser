  ...
  async execute(interaction) {
    ...
    if (cooldownUsers.has(user.id)) {
      return interaction.reply({
        content: "⏳ Chill for a bit, you're on cooldown.",
        ephemeral: true,
      });
    }
    ...
    const embed = new EmbedBuilder()
      .setColor("Blurple")
      .setTitle(
        `${user.username} rolled ${count} ${
          count === 1 ? `d${faces}` : `d${faces}'s`
        }`
      )
      .setDescription(
        `${
          rollingEmojis[Math.floor(Math.random() * rollingEmojis.length)]
        } Rolling... ${
          rollingEmojis[Math.floor(Math.random() * rollingEmojis.length)]
        }`
      );
