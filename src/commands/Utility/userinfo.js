...
const { Nickname, Username } = require("../../database/index");
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      const [usernameData, nicknameData, joinPosition, profileBuffer] =
        await Promise.all([
          Username.findOne({ where: { discordId: member.id } }),
          Nickname.findOne({
            where: {
              discordId: member.id,
      ...
      const boostSince = member.premiumSince
        ? `<t:${Math.floor(
            ...
            (Date.now() - member.premiumSince.getTime()) / 86400000
          )} days ago)`
        : "Not boosting";
      ...
      const displayedRoles =
        rolesArray.length > 20
          ? rolesArray.slice(0, 20).join(", ") +
            `... (+${rolesArray.length - 20} more)`
          : rolesArray.join(", ");
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          },
          {
            ...
            value: `<t:${createdAtUnix}:f> (${Math.round(
          ...
          {
            name: "Joined Server",
            value: `<t:${joinedAtUnix}:f> (${Math.round(
          ...
          {
            name: "Join Position",
            value: `${member.displayName} was the ${addSuffix(
              joinPosition
          ...
          {
            name: `Roles [${rolesArray.length}]`,
            value: displayedRoles || "No roles assigned",
            inline: false,
          },
          {
      ...
      await interaction.editReply({
        content: `\`❌\` There was an error generating the info for **${member.user.tag}**.`,
      });
    }
  },
};
